...
export default function HomeTabGallery({ onStatusChange }: HomeTabGalleryProps) {
    ...
    const handleDeleteImage = async (imageName: string) => {
        ...
        try {
            const response = await fetch(
                `/api/delete?path=${encodeURIComponent('home/gallery')}&name=${encodeURIComponent(imageName)}`,
                {
                    method: 'DELETE',
                },
            );
    ...
    const handleDeleteAllImages = async () => {
        if (
            !confirm(
                `Êtes-vous sûr de vouloir supprimer toutes les photos (${images.length} images) ? Cette action est irréversible.`,
            )
        ) {
            return;
        }
        ...
        try {
            ...
            await Promise.all(
                images.map((image) =>
                    fetch(
                        `/api/delete?path=${encodeURIComponent('home/gallery')}&name=${encodeURIComponent(image.name)}`,
                        {
                            method: 'DELETE',
                        },
                    ),
        ...
        <div>
            ...
            <div className="bg-white rounded-lg shadow p-6">
                ...
                >
                    ...
                    <p className="mt-1 text-xs text-gray-500">PNG, JPG, GIF, WEBP acceptés</p>
                </div>
