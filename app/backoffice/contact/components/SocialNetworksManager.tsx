'use client';

import { useState } from 'react';
import { v4 as uuidv4 } from 'uuid';

import { SocialNetwork, SocialNetworkType } from '../../models/contactTypes';

interface SocialNetworksManagerProps {
    readonly socialNetworks: SocialNetwork[];
    readonly onChange: (networks: SocialNetwork[]) => void;
}

// Configuration des icônes et noms pour chaque type de réseau social
const SOCIAL_NETWORK_CONFIG: Record<
    SocialNetworkType,
    { icon: string; defaultName: string; placeholder: string }
> = {
    instagram: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>`,
        defaultName: 'Instagram',
        placeholder: 'https://instagram.com/votre-compte',
    },
    facebook: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>`,
        defaultName: 'Facebook',
        placeholder: 'https://facebook.com/votre-page',
    },
    twitter: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
              </svg>`,
        defaultName: 'Twitter',
        placeholder: 'https://twitter.com/votre-compte',
    },
    linkedin: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>`,
        defaultName: 'LinkedIn',
        placeholder: 'https://linkedin.com/in/votre-compte',
    },
    tiktok: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.23-3.02.37-.63.41-1.11 1.04-1.36 1.75-.21.51-.15 1.07-.14 1.61.24 1.64 1.82 3.02 3.5 2.87 1.12-.01 2.19-.66 2.77-1.61.19-.33.4-.67.41-1.06.1-1.79.06-3.57.07-5.36.01-4.03-.01-8.05.02-12.07z"/>
              </svg>`,
        defaultName: 'TikTok',
        placeholder: 'https://tiktok.com/@votre-compte',
    },
    youtube: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
              </svg>`,
        defaultName: 'YouTube',
        placeholder: 'https://youtube.com/@votre-chaine',
    },
    snapchat: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.748-1.378 0 0-.599 2.282-.744 2.840-.282 1.084-1.064 2.456-1.549 3.235C9.584 23.815 10.77 24.001 12.017 24.001c6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641.001 12.017.001z"/>
              </svg>`,
        defaultName: 'Snapchat',
        placeholder: 'https://snapchat.com/add/votre-compte',
    },
    pinterest: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.748-1.378 0 0-.599 2.282-.744 2.840-.282 1.084-1.064 2.456-1.549 3.235C9.584 23.815 10.77 24.001 12.017 24.001c6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641.001 12.017.001z"/>
              </svg>`,
        defaultName: 'Pinterest',
        placeholder: 'https://pinterest.com/votre-compte',
    },
    whatsapp: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
              </svg>`,
        defaultName: 'WhatsApp',
        placeholder: 'https://wa.me/votre-numero',
    },
    telegram: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
              </svg>`,
        defaultName: 'Telegram',
        placeholder: 'https://t.me/votre-compte',
    },
    website: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm0 2.4c2.123 0 4.08.693 5.663 1.863L16.8 5.4c-1.403-.747-3.072-1.2-4.8-1.2s-3.397.453-4.8 1.2L6.337 4.263C7.92 3.093 9.877 2.4 12 2.4zm-7.2 7.2h3.6c.133-1.547.453-3.04.92-4.463L7.577 3.394C5.697 4.667 4.373 6.747 4.053 9.6h.747zm0 2.4H4.8c.32 2.853 1.644 4.933 3.524 6.206l1.743-1.743c-.467-1.423-.787-2.916-.92-4.463H4.8zm7.2 9.6c-2.123 0-4.08-.693-5.663-1.863L7.2 18.6c1.403.747 3.072 1.2 4.8 1.2s3.397-.453 4.8-1.2l.863 1.137C16.08 20.907 14.123 21.6 12 21.6zm7.2-7.2h-3.6c-.133 1.547-.453 3.04-.92 4.463l1.743 1.743c1.88-1.273 3.204-3.353 3.524-6.206H19.2zm0-2.4h.747c-.32-2.853-1.644-4.933-3.524-6.206l-1.743 1.743c.467 1.423.787 2.916.92 4.463H19.2z"/>
              </svg>`,
        defaultName: 'Site Web',
        placeholder: 'https://votre-site.com',
    },
    other: {
        icon: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>
              </svg>`,
        defaultName: 'Autre',
        placeholder: 'https://votre-lien.com',
    },
};

// Fonction utilitaire pour normaliser les URLs
const normalizeUrl = (url: string): string => {
    if (!url) return '';

    // Si l'URL commence déjà par http:// ou https://, on la retourne telle quelle
    if (url.startsWith('http://') || url.startsWith('https://')) {
        return url;
    }

    // Sinon, on ajoute https:// par défaut
    return `https://${url}`;
};

export default function SocialNetworksManager({
    socialNetworks = [],
    onChange,
}: SocialNetworksManagerProps) {
    const [showAddForm, setShowAddForm] = useState(false);
    const [newNetwork, setNewNetwork] = useState<Partial<SocialNetwork>>({
        type: 'instagram',
        name: '',
        url: '',
        displayName: '',
    });

    const handleAddNetwork = () => {
        if (newNetwork.type && newNetwork.url && newNetwork.displayName) {
            const network: SocialNetwork = {
                id: uuidv4(),
                type: newNetwork.type,
                name: newNetwork.name || SOCIAL_NETWORK_CONFIG[newNetwork.type].defaultName,
                url: normalizeUrl(newNetwork.url),
                displayName: newNetwork.displayName,
            };

            onChange([...(Array.isArray(socialNetworks) ? socialNetworks : []), network]);
            setNewNetwork({
                type: 'instagram',
                name: '',
                url: '',
                displayName: '',
            });
            setShowAddForm(false);
        }
    };

    const handleUpdateNetwork = (id: string, field: keyof SocialNetwork, value: string) => {
        if (!Array.isArray(socialNetworks)) return;

        // Normaliser l'URL si c'est le champ url qui est modifié
        const normalizedValue = field === 'url' ? normalizeUrl(value) : value;

        const updatedNetworks = socialNetworks.map((network) =>
            network.id === id ? { ...network, [field]: normalizedValue } : network,
        );
        onChange(updatedNetworks);
    };

    const handleDeleteNetwork = (id: string) => {
        if (!Array.isArray(socialNetworks)) return;

        const updatedNetworks = socialNetworks.filter((network) => network.id !== id);
        onChange(updatedNetworks);
    };

    const handleTypeChange = (type: SocialNetworkType) => {
        const config = SOCIAL_NETWORK_CONFIG[type];
        setNewNetwork({
            ...newNetwork,
            type,
            name: config.defaultName,
        });
    };

    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                <h3 className="text-lg font-medium text-gray-900">Réseaux sociaux</h3>
                <button
                    type="button"
                    onClick={() => setShowAddForm(true)}
                    className="px-3 py-1 bg-black text-white text-sm rounded-md hover:bg-gray-800 transition-colors"
                >
                    Ajouter un réseau
                </button>
            </div>

            {/* Liste des réseaux existants */}
            <div className="space-y-3">
                {Array.isArray(socialNetworks)
                    ? socialNetworks.map((network) => {
                          const config = SOCIAL_NETWORK_CONFIG[network.type];
                          return (
                              <div
                                  key={network.id}
                                  className="flex items-center space-x-3 p-3 border border-gray-200 rounded-md"
                              >
                                  <div
                                      className="w-6 h-6 flex-shrink-0"
                                      dangerouslySetInnerHTML={{ __html: config.icon }}
                                  />

                                  <div className="flex-grow grid grid-cols-1 md:grid-cols-3 gap-2">
                                      <input
                                          type="text"
                                          value={network.displayName}
                                          onChange={(e) =>
                                              handleUpdateNetwork(
                                                  network.id,
                                                  'displayName',
                                                  e.target.value,
                                              )
                                          }
                                          placeholder="Nom affiché"
                                          className="px-2 py-1 border border-gray-300 rounded text-sm"
                                      />
                                      <input
                                          type="text"
                                          value={network.url}
                                          onChange={(e) =>
                                              handleUpdateNetwork(network.id, 'url', e.target.value)
                                          }
                                          placeholder={config.placeholder}
                                          className="px-2 py-1 border border-gray-300 rounded text-sm"
                                      />
                                      <div className="flex items-center space-x-2">
                                          <select
                                              value={network.type}
                                              onChange={(e) =>
                                                  handleUpdateNetwork(
                                                      network.id,
                                                      'type',
                                                      e.target.value as SocialNetworkType,
                                                  )
                                              }
                                              className="px-2 py-1 border border-gray-300 rounded text-sm flex-grow"
                                          >
                                              {Object.entries(SOCIAL_NETWORK_CONFIG).map(
                                                  ([type, config]) => (
                                                      <option key={type} value={type}>
                                                          {config.defaultName}
                                                      </option>
                                                  ),
                                              )}
                                          </select>
                                      </div>
                                  </div>

                                  <button
                                      type="button"
                                      onClick={() => handleDeleteNetwork(network.id)}
                                      className="text-red-500 hover:text-red-700 p-1"
                                      aria-label="Supprimer"
                                  >
                                      <svg
                                          className="w-4 h-4"
                                          fill="none"
                                          stroke="currentColor"
                                          viewBox="0 0 24 24"
                                      >
                                          <path
                                              strokeLinecap="round"
                                              strokeLinejoin="round"
                                              strokeWidth={2}
                                              d="M6 18L18 6M6 6l12 12"
                                          />
                                      </svg>
                                  </button>
                              </div>
                          );
                      })
                    : null}
            </div>

            {/* Formulaire d'ajout */}
            {showAddForm && (
                <div className="p-4 border border-gray-200 rounded-md bg-gray-50">
                    <h4 className="font-medium text-gray-900 mb-3">Ajouter un réseau social</h4>

                    <div className="space-y-3">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                Type de réseau
                            </label>
                            <select
                                value={newNetwork.type}
                                onChange={(e) =>
                                    handleTypeChange(e.target.value as SocialNetworkType)
                                }
                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                            >
                                {Object.entries(SOCIAL_NETWORK_CONFIG).map(([type, config]) => (
                                    <option key={type} value={type}>
                                        {config.defaultName}
                                    </option>
                                ))}
                            </select>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                Nom affiché (ex: @votre-compte)
                            </label>
                            <input
                                type="text"
                                value={newNetwork.displayName}
                                onChange={(e) =>
                                    setNewNetwork({ ...newNetwork, displayName: e.target.value })
                                }
                                placeholder="@dalifilms.fr"
                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                                required
                            />
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                URL
                            </label>
                            <input
                                type="text"
                                value={newNetwork.url}
                                onChange={(e) =>
                                    setNewNetwork({ ...newNetwork, url: e.target.value })
                                }
                                placeholder={
                                    newNetwork.type
                                        ? SOCIAL_NETWORK_CONFIG[newNetwork.type].placeholder
                                        : ''
                                }
                                className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm"
                                required
                            />
                            <p className="text-xs text-gray-500 mt-1">
                                Vous pouvez saisir l&apos;URL avec ou sans https:// (sera ajouté
                                automatiquement)
                            </p>
                        </div>

                        <div className="flex space-x-2">
                            <button
                                type="button"
                                onClick={handleAddNetwork}
                                className="px-4 py-2 bg-black text-white text-sm rounded-md hover:bg-gray-800 transition-colors"
                            >
                                Ajouter
                            </button>
                            <button
                                type="button"
                                onClick={() => setShowAddForm(false)}
                                className="px-4 py-2 bg-gray-200 text-gray-800 text-sm rounded-md hover:bg-gray-300 transition-colors"
                            >
                                Annuler
                            </button>
                        </div>
                    </div>
                </div>
            )}
        </div>
    );
}
