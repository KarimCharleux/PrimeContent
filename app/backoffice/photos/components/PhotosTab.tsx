        ...
        try {
            ...
            const promises = photos.map(async (photo) => {
                try {
                    ...
                    const filename = photo.source.split('/').pop() || `photo-${photo.id}.jpg`;
                    folder?.file(filename, blob);
                } catch (e) {
                    console.error('Erreur téléchargement:', photo.source, e);
                } finally {
                    completed++;
                    setDownloadProgress(Math.round((completed / photos.length) * 100));
                }
            });
            ...
            setStatusMessage({ type: 'success', message: 'Téléchargement terminé' });
        } catch (e) {
            setStatusMessage({ type: 'error', message: 'Erreur lors de la création du ZIP' });
        } finally {
