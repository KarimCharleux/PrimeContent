...
export function AuthProvider({ children }: AuthProviderProps) {
    ...
    const getUserProfile = useCallback(
        async (uid: string, fbUser?: FirebaseUser | null, maxRetries = 2): Promise<User | null> => {
            let retries = 0;
            ...
            const fetchWithRetry = async (): Promise<User | null> => {
                try {
                    ...
                    const timeoutPromise = new Promise<null>((_, reject) => {
                        setTimeout(
                            () => reject(new Error('Timeout lors de la récupération du profil')),
                            5000,
                        );
                    });

                    // Récupérer le profil utilisateur
                    const fetchProfilePromise = getDoc(doc(db, 'users', uid)).then((docSnap) => {
                        if (docSnap.exists()) {
                            ...
                            setDoc(doc(db, 'users', uid), defaultProfile).catch((err) =>
                    ...
                    if (retries < maxRetries) {
                        retries++;
                        console.log(
                            `Tentative ${retries}/${maxRetries} pour récupérer le profil...`,
                        );
                        ...
                        console.error(
                            'Erreur lors de la récupération du profil après plusieurs tentatives:',
                            error,
                        );
